...
// res.on('finish', _finishReqResCycle);
// if route is missing, it's a route error, or an error before routing occurred
/**
...
function _finishReqResCycle(req, res, route, err) {
    ...
    // if the response has not yet finished, don't emit the after event
    // yet even though we've reached this point. this handles async
    // formatter errors. res.finished is set by node core's response
    // object, when res.end() completes.
