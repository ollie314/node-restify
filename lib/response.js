...
Response.prototype.send = function send(code, body, headers) {
    ...
    function _cb(err, _body) {
        ...
        if (err) {
            ...
            // set formatter error flag so server can use it in the after event.
            // check if this is an instance of Error - if not, wrap in restify
            // InternalServerError event.
            self.formatterErr = (err instanceof Error) ?
                err : new InternalServerError('formatter error');
        } else {
            self._data = _body;
        }
